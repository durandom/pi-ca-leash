...
test("parseSubagentRunToolInput rejects missing task and invalid driver", () => {
  ...
  assert.throws(() => parseSubagentRunToolInput({ task: "x", driver: "wat" }), /driver must be claude-sdk, claude-cli, claude-pty, codex-cli, or pi-coding-agent/);
});
...
test("parseTeamSpawnToolInput rejects missing required fields and invalid driver", () => {
  ...
  assert.throws(() => parseTeamSpawnToolInput({ name: "worker", prompt: "hi", driver: "wat" }), /driver must be claude-sdk, claude-cli, claude-pty, codex-cli, or pi-coding-agent/);
});
