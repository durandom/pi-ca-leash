...
export function parsePeerStartCommandInput(args: string): ParsedPeerStartCommandInput {
  ...
  if (parts.length === 3) {
    ...
    if (!driver) {
      throw new Error("driver must be claude-sdk, claude-cli, claude-pty, codex-cli, pi-coding-agent, or agy when using a driver-aware peer start form");
    }
    return {
      name: parts[0],
      prompt: parts[1] ?? "",
  ...
  const driver = parseRuntimeDriverName(parts[2]);
  if (!driver) {
    throw new Error("driver must be claude-sdk, claude-cli, claude-pty, codex-cli, pi-coding-agent, or agy when using <name> | <prompt> | <driver> | <model>");
  }
  return {
    name: parts[0],
    prompt: parts[1] ?? "",
...
export function parseSubagentRunCommandInput(args: string): ParsedSubagentRunCommandInput {
  ...
  const driver = parseRuntimeDriverName(parts[0]);
  if (!driver) {
    throw new Error("driver must be claude-sdk, claude-cli, claude-pty, codex-cli, pi-coding-agent, or agy when using <driver> | <task>");
  }
  return {
    driver,
...
export function parseTeamSpawnCommandInput(args: string): ParsedTeamSpawnCommandInput {
  ...
  if (!driver) {
    throw new Error("driver must be claude-sdk, claude-cli, claude-pty, codex-cli, pi-coding-agent, or agy when using <name> | <prompt> | <driver>");
  }
  return {
    name: name ?? "",
    prompt: prompt ?? "",
